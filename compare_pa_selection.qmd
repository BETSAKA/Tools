...
pa_sapm <- st_read("data/SAPM 2017/SAPM_2017_corrected.shp") %>%
  st_make_valid() %>%
  st_transform(4326)
...
The three WUs do not use the same final PA register. WU 1.1 contains the broadest list, WU 2.1 the narrowest one, and the common intersection should be treated as the stable core list shared across all analyses.
...
pa_list_inconsistencies
```

### Differences in PA names or establishment years
...
pa_metadata_inconsistencies <- pa_comparison |>
  ...
  rowwise() |>
  mutate(
    n_distinct_names = n_distinct(na.omit(c(`NAME_STD_WU 1.1`, `NAME_STD_WU 1.2`, `NAME_STD_WU 2.1`))),
    n_distinct_years = n_distinct(na.omit(c(`YEAR_WU 1.1`, `YEAR_WU 1.2`, `YEAR_WU 2.1`)))
  ) |>
  ungroup() |>
  filter(n_distinct_names > 1 | n_distinct_years > 1) |>
  transmute(
    WDPAID,
    NAME_WU11 = `NAME_WU 1.1`,
    NAME_WU12 = `NAME_WU 1.2`,
  ...
  arrange(desc(n_distinct_years), desc(n_distinct_names), WDPAID)

pa_metadata_inconsistencies
```

The table above flags the PAs for which the three WUs disagree on either the PA label or the year of establishment. Rows with `n_distinct_years > 1` are the most urgent because they imply different treatment timing across analyses, while rows with `n_distinct_names > 1` are mainly harmonization issues that can still break joins and comparisons.

### Year inconsistencies to correct first

```{r}
year_inconsistencies <- pa_metadata_inconsistencies |>
